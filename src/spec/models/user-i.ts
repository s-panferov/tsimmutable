...
import * as profile from './profile-i';
import * as acl from './acl-i';

/**
* Map interface for User with specialized getters and setters.
*/

export interface UserMap extends ImmutableMethods<string, any, UserMap> {

    $UserMap: UserMap

    get(key: 'profile', defaultValue?: profile.ProfileMap): profile.ProfileMap
    set(key: 'profile', value: profile.ProfileMap): UserMap
    ...
    get(key: 'acl', defaultValue?: acl.AclMap): acl.AclMap
    set(key: 'acl', value: acl.AclMap): UserMap
...
export interface UserRecordShape extends ImmutableMethods<string, any, UserRecordShape> {

    $UserRecordShape: UserRecordShape

    profile: profile.ProfileRecord
    get(key: 'profile', defaultValue?: profile.ProfileRecord): profile.ProfileRecord
    set(key: 'profile', value: profile.ProfileRecord): UserRecordShape

    login: string
    get(key: 'login', defaultValue?: string): string
    set(key: 'login', value: string): UserRecordShape

    friends: Immutable.List<UserRecord>
    get(key: 'friends', defaultValue?: Immutable.List<UserRecord>): Immutable.List<UserRecord>
    set(key: 'friends', value: Immutable.List<UserRecord>): UserRecordShape

    type: UserType
    get(key: 'type', defaultValue?: UserType): UserType
    set(key: 'type', value: UserType): UserRecordShape

    acl: acl.AclRecord
    get(key: 'acl', defaultValue?: acl.AclRecord): acl.AclRecord
    set(key: 'acl', value: acl.AclRecord): UserRecordShape


    get(key: string, defaultValue?: any): any;
    set(key: string, value: typeof undefined): UserRecordShape;
 ...
 */
export let UserRecordCtor: RecordCtor<UserRecordDefaults, UserRecordShape> = Immutable.Record as any;

/**
 * Special method to parse UserRecord with all the dependencies.
 */
export function parseUserRecord(value: User, deps: any): UserRecordShape {
    var recordWalker = function(value, key) {
        switch (true) {

            case key == 'profile':
                if (value != null) {
                    return deps.ProfileRecord.fromJS(value, deps);
                } else {
                    return undefined;
                }
            ...
            case key == 'friends':
                if (value != null) {
                    return Immutable.List(value.map((item) => {
                        return deps.UserRecord.fromJS(item, deps);
                    }));
                } else {
                    return undefined;
                }
            ...
            case key == 'acl':
                if (value != null) {
                    return deps.AclRecord.fromJS(value, deps);
                } else {
                    return undefined;
                }
...
export class UserRecord extends UserRecordCtor({
    ...
    static fromJS(value: User, deps: any): UserRecord {
        return parseUserRecord(value, deps)
    }
}
